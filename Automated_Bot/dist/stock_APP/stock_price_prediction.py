    ...
    def Technical_Automated(self,data):
        ...
        for i in range(len(data)):
            ...
            if i>3:
                ...
                buy_condition_1_risky = data["Volume"][i] > data["Volume"][i-1]*2 and (data["DIFF"][i-1] + data["DIFF"][i] <0  ) and (data["Close"][i]<= data["BOLL_LB"][i]+data["BOLL_STD"][i] ) and data["Close"][i-1] <data["Close"][i] and data["SMA"][i-1]<=data["SMA"][i]#and data["DIFF"][i]<=data["DIFF"][i-1])
                
                buy_condition_2_risky = data["SMA"][i-1]<=data["SMA"][i] and data["Close"][i-1]<data["Close"][i] and ( data["DIFF"][i]>=0 ) and (data["Close"][i]<= data["BOLL_LB"][i]+data["BOLL_STD"][i]   or (data["Close"][i]> data["SMA"][i]+ data["BOLL_STD"][i] and data["Close"][i-1]<data["SMA"][i-1]+ data["BOLL_STD"][i-1]) ) # and (data["Close"][i-1]<data["Close"][i] and data["Close"][i-2]>data["Close"][i-1]) 

                #buy_condition_3_risky = data["Close"][i-1]<data["BOLL_LB"][i-1]+data["BOLL_STD"][i-1] and data["Close"][i]<data["BOLL_LB"][i]+data["BOLL_STD"][i]

                normal_buy_condition = data["Close"][i-1] <= data["BOLL_LB"][i-1] and data["Close"][i] > data["BOLL_LB"][i] 
                ...
                if decision == 0:
                    ...
                    if ((data["Close"][i-1] <= data["BOLL_LB"][i-1] and data["Close"][i] > data["BOLL_LB"][i] and  data["DIFF"][i] > 0) or buy_condition_1_risky or buy_condition_2_risky): 
                    ...
                    if data["Close"][i]<data["SMA"][i]:
                        EMA_count+=1
                    if (data["Close"][i-1] > data["BOLL_UB"][i-1] and data["Close"][i] < data["BOLL_UB"][i] and  data["MACD"][i] < data["Signal"][i]) or ( data["Close"][i-1] > data["SMA"][i-1] ):#or data["Close"][i-1] > data["Close"][i]:#or  data["Close"][i] <= data["BOLL_LB"][i] : #and data["MACD"][i-1] > data["MACD"][i]:#and 
